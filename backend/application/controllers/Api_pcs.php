...
class Api_pcs extends REST_Controller
{
    ...
    private $secret_key = "batu lu babi";
    ...
    public function login_post()
    {
        ...
        if (empty($result)) {
            ...
            $payload["exp"] = $date->getTimestamp() + 10000000; //expired 1 jam(3600 s)
    ...
    //tambahan
    public function produkbeli_get()
    {
        // $a = $_SESSION['b'];
        $this->cekToken();
        $data = $this->M_produk->get_produkbeli();
        $result = array(
            "success" => true,
            "message" => "Data Correct",
            "data" => array("produk" => $data) //tambahan
        );
        echo json_encode($result);
    }

    //tambahan
    public function produkjual_get()
    {
        // $a = $_SESSION['b'];
        $this->cekToken();
        $data = $this->M_produk->get_produkjual();
        $result = array(
            "success" => true,
            "message" => "Data Correct",
            "data" => array("produk" => $data) //tambahan
    ...
    public function produktambah_post()
    {
        $this->cekToken();
        $validation_message = [];
        ...
        if ($this->post("stokjual") == "") {
            array_push($validation_message, "stok tidak boleh kosong");
        }
        if ($this->post("stokjual") != "" && !is_numeric($this->post("stok"))) {
        ...
        $insert = $this->M_produk->insert_produktambah($data);
    ...
    // public function produkbeli_post()
    // {
    //     $this->cekToken();
    //     $validation_message = [];
    ...
    //         'harga' => $this->post('harga'),
    //         'stok' => $this->post('stok'),
    //         'stokjual' => $this->post('stokjual'),
    //         'status' => 1
    //     );

    //     $id = $this->post('id');
    //     $insert = $this->M_produk->insert_produkbeli($data, $id);
    ...
    public function produkbeli_put()
    {
        $this->cekToken();
        $validation_message = [];
        ...
        $result = $this->M_produk->update_produkbeli($data, $id);
    ...
    public function produkjual_put()
    {
        $this->cekToken();
        $validation_message = [];
        ...
        $result = $this->M_produk->update_produkjual($data, $id);
    ...
    //tambahan
    public function produkjualdelete_put()
    {
        $this->cekToken();
        $validation_message = [];
        ...
        $result = $this->M_produk->update_produkjualdelete($data, $id);
    ...
    public function transaksi_post()
    {
        ...
        $result = $this->M_transaksi->insert_transaksi($data);
    ...
    public function item_transaksi_get()
    {
        ...
        $result = array(
            "success" => true,
            "message" => "Data Correct",
            "data" => array("item_transaksi" => $data)
        );
        echo json_encode($result);
    }
