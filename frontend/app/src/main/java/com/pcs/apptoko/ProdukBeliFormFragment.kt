...
class ProdukBeliFormFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val view = inflater.inflate(R.layout.fragment_produkbeli_form, container, false)

        val btnBeliProduk = view.findViewById<Button>(R.id.btnProsesProduk)
...
//            if(status=="edit"){
//                api.putProduk(token.toString(),produk?.id.toString().toInt(),adminId.toString().toInt(),txtFormNama.text.toString(),txtFormHarga.text.toString().toInt(),
//                    txtFormStok.text.toString().toInt(), 1).enqueue(object :
...
//            }
//            else{
                api.postProdukTambah(token.toString(),adminId.toString().toInt(),txtFormNama.text.toString(),txtFormHarga.text.toString().toInt(),
                    txtFormStok.text.toString().toInt(),0, 0).enqueue(object :
                    Callback<ProdukResponsePost> {
                    override fun onResponse(
                        call: Call<ProdukResponsePost>,
                        response: Response<ProdukResponsePost>
